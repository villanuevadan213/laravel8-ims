<x-layout>
    ...
    <div class="container mx-auto mt-6 px-4 sm:px-6 lg:px-8">
        <!-- Inventory Overview -->
        <div
            class="bg-white shadow-lg rounded-lg p-4 sm:p-6 mb-6 border border-gray-200 hover:shadow-xl transition-shadow duration-300">
            <h3 class="text-xl sm:text-2xl font-semibold text-gray-900 mb-4 flex items-center">
                <svg class="w-5 h-5 sm:w-6 sm:h-6 mr-2 text-blue-500" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
            ...
            @if($lowStock->count())
                <ul class="list-disc pl-4 sm:pl-6 space-y-2 text-gray-700">
                    @foreach ($lowStock as $product)
                        <li
                            class="flex flex-col sm:flex-row justify-between sm:items-center hover:bg-gray-50 transition-colors p-2 rounded-lg">
                            <div>
